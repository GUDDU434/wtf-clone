...
//   console.log(gym_id);
  ...
  const { gymDetails, gymPlanData, isLoading,loadingPlan } = useSelector((state) => state);

//   console.log(gymDetails, gymPlanData, isLoading);
  ...
  return (
    <div>
      <div className={styles.gymImageSlider}><img src="" alt="Sliding image" /></div>
      ...
      <div className={styles.bodyGymdetails}>
        ...
        <div className={styles.gymPlan}>
          {planLoading ? (
            ...
            plan.map((elem,index) => <PlanCard key={elem.id} {...elem} index={index} />)
          )}
        </div>
      </div>
    </div>
